...
## 附录 C. 数据采集与 AVP 视角(现状 + 待补)

### C.1 AVP 数据方向(澄清一个易错点)

- **上行(AVP → 电脑)**:AVP 用 ARKit 追踪**操作者本人**的头 6DoF + 双手手腕位姿 + 手指 keypoints,`avp_publisher` 抓取经 UDP 发出。**这是人的手,不是机器人的手**——是遥操的**输入**。
- **下行(机器人 → AVP)**:机器人头部相机画面可经官方栈(`camera_driver` + WebRTC/H264)**投屏回 AVP 屏幕**,让操作者"透过机器人的眼睛看"。这是给人看的视频,不是追踪数据。
- **训练数据的正确取向**:模仿学习要的是"机器人视角图像 + 机器人本体状态"(观测)→ "机器人动作命令"(动作)。**操作者自己的手(AVP 原始数据)一般不是训练标签**,只是生成机器人动作的中间输入。
- **重构诉求(GPT prompt 硬约束 6)**:AVP 需支持**视角切换** —— robot-view(看机器人相机)/ self-view(AVP passthrough 看真实环境)。

### C.2 训练数据录制现状

- **我们的现状**:`trajectory_io.py`(§8)只为回放/调试设计,自定义 JSON,**无视觉观测、非通用格式、无多模态时间对齐**,不能直接训练。
- **官方录制**:`teleop_official` 真机端录制实现 —— **调查中,结果见下**(填充后)。
- **重构诉求(GPT prompt 硬约束 7)**:需独立的数据采集节点(机器人视角图像 + `joint_space_states` + IK 动作命令 + 时间同步 → LeRobot/RLDS/HDF5-episodic)。

### C.3 官方数据录制管线(调查结果 —— 重要:应复用而非自建)

**结论:官方已有一套完整、成熟的模仿学习数据采集管线**(不是简单 rosbag 调试)。作者署名 `lukexie@astribot.com`(2025),开机自动拉起。

**组件**:
- `astribot_recorder` 包:`astribot_recorder_server`(C++,ARM,录制服务)+ `astribot_recorder_trigger` + `web_old_communication.py`(网页编排开始/停止/保存)。
- `astribot_data_processor` 包:`astribot_data_processor_tool`(C++,MCAP→HDF5 转换器)。
- 定制 rosbag2 后端(`astribot_rosbag2_storage/transport`)、PTP 时间同步(`ptp_sync_time`)、按容量清理(`system_monitor`,15G 上限)。

**录制内容(视觉 + 状态 + 命令三者齐全,时间对齐)**:
- **相机**:5 路 RGB + 4 路深度,30Hz,`sensor_msgs/CompressedImage`——头部 RGBD(1280×720)、胸/躯干、左右腕(640×360)、头部立体(1920×1080)。话题 `/astribot_camera/{head_rgbd,torso_rgbd,left_wrist_rgbd,right_wrist_rgbd,head_stereo}/color_compress/compressed`。
- **关节状态(obs)**:`joint_space_states`,**250Hz**,`RobotJointState`(pos/vel/torque),7 个部位分开。
- **动作命令(action)**:`joint_space_command`,**100Hz**,`RobotJointController`,7 部位。
- **末端**:`pose_command_array`(WBC 输出末端命令)、`endpoint_current_states`(末端位姿/力)、各连杆 TF。
- **时间同步**:PTP 双 SOC 同步;转换器保留 header/bag/timestamp 三种时间戳。

**输出格式**:MCAP(原始,双 SOC 各一份)→ **HDF5**(训练用,`{task}_episode_{N}.hdf5`)。HDF5 内部 `images_dict/{head,left,right}/rgb` + `joints_dict/<part>/{states,command}` + `command_poses_dict` + `endpoint_wrench_dict`——**典型 ACT/ALOHA 系 episodic 布局**。非 LeRobot/RLDS 原生,但转过去只需一层字段映射。

**控制录制**:ROS2 service `/astribot_storage/storage_service`(`StorageService`)+ action `Storage`,指令码 `TYPE_START_RECORD=1 / STOP=2 / SAVE=6 / DELETE=7`;实际由 VR 网页触发。存 `/home/astribot/workspace/storage/`(MCAP 归档 + HDF5 输出 + NFS 挂载)。

**对我们的关键含义**:
- 官方 recorder 录的 action 标签就是 `joint_space_command` 话题——而**我们自研 IK 正是往这个话题发命令**(附录 A.1)。所以只要我们直接发布到 `/astribot_<部件>/joint_space_command`,**官方 recorder 会自动把我们的关节命令当 action 标签录下来**,连同机器人相机图像 + 关节状态一起打包成 HDF5。这意味着**训练数据采集大概率应直接复用官方管线,而不是自建**(自建拿不到真实相机流 + 硬件时间戳)。
- 需向官方/真机确认的:我们绕过 WBC 直发关节命令时,`joint_space_command` 上录到的是否就是我们发的值(而非 WBC 改写后的);以及 `pose_command_array`(WBC 末端命令)在我们不走 WBC 时是否为空。
- C++ 录制服务与转换器都是 ARM 二进制,只在机器人本体跑;仓库有完整源码头/配置/launch,但 macOS 无法本地执行(仿真阶段的数据采集仍需我们自建一个等价的 episodic recorder)。

---

## 附录 B. 重构相关的决策(与本报告配套的 GPT prompt 对应)
...
6. **AVP 视角可切换** —— 见附录 C.1,robot-view / self-view 切换。
7. **训练数据采集节点** —— 见附录 C.2,机器人视角图像 + 状态 + 动作,通用 episodic 格式。
